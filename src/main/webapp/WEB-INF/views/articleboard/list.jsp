						...
						<div class="md3">
							<select class="form-control search-select" name="condition">
								<option value="article_title"
									${pc.paging.condition == 'article_title' ? 'selected' : '' }>제목</option>
								<option value="article_content"
									${pc.paging.condition == 'article_content' ? 'selected' : '' }>내용</option>
								<option value="article_type"
									${pc.paging.condition == 'article_type' ? 'selected' : '' }>
									기관 종류</option>
							</select>
						</div>
					...
					<tbody style="text-align: center;">
						<c:forEach var="vo" items="${articleList}">
							<tr>
								...
								<td><a
									href="<c:url value='/articleboard/view/${vo.article_id}${pc.makeURI(pc.paging.pageNum)}' />">
										${vo.article_title}</a> &nbsp;&nbsp;&nbsp;</td>
								<td><fmt:formatDate value="${vo.article_start_date}"
										pattern="yyyy-MM-dd " /></td>
								<td><fmt:formatDate value="${vo.article_expired_date}"
										pattern="yyyy-MM-dd " /></td>
								<c:if test="${login.type == 1}">
									<td><fmt:formatDate value="${vo.article_regdate}"
											pattern="yyyy-MM-dd" /></td>
					...
					<div class="text-center clearfix">
						...
						<c:if test="${login.type==1 }">
							...
							<button type="button" class="btn btn-info"
								onclick="location.href='<c:url value="/articleboard/write" />'">글
								등록</button>
						</c:if>
					</div>
